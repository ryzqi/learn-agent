...
// Agent Loop 核心：协调模型、工具、权限、Hook、后台事件与资源关闭，并保证每个工具调用都有协议回填。
...
// Harness 托管的异步资源统一暴露关闭协议，Agent 退出时按注册顺序释放。
...
// AgentRunner 是完整 Harness 的执行边界：串联提示词、工具、事件泵和托管资源生命周期。
export class AgentRunner {
  ...
  // 每轮模型请求前组装 request 与 tools；处理器可对 history 做请求级压缩，
  // turn/observer 指导只注入本次请求，不写入 persistent history。
  async #prepareModelRequest(): Promise<{
    ...
    const turnGuidance = this.#turnLifecycle === undefined ? [] : this.#turnLifecycle.beforeModel();
    if (
  ...
  // 事件泵出口：按 eventId 去重，context/mailbox 事件推迟到用户轮次，只有被接受的事件才 ack。
